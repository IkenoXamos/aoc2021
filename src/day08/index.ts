...
function hasUniqueLength(part: string): boolean {
  return part.length === 2 || // 1
    part.length === 3 || // 7
    part.length === 4 || // 4
    part.length === 7; // 8
}
...
const part1 = (rawInput: string) => {
  ...
  return input.reduce<number>((acc, line) => acc + line[1].reduce<number>((acc, part) => acc + (hasUniqueLength(part) ? 1 : 0), 0), 0);
};
