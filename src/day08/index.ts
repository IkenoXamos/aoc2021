...
function deduce_invalid_mappings(connections: boolean[][], part: string, validIndexes: number[]): void {
  ...
  for(let i = 0; i < 7; i++) {
    ...
    for(let j = 0; j < 7; j++) {
      ...
      console.log(`Analyzing ${part} -> ${validIndexes.map( val => String.fromCharCode(97 + val)).reduce<string>( (acc, letter) => acc + letter, '')}`);
      console.log(`Deducing that ${String.fromCharCode(i + 97)} is not ${String.fromCharCode(j + 97)}`)
      // console.log(`Deducing that ${i} is not ${j}`)
...
const part2 = (rawInput: string) => {
  ...
  for(const display of input) {
    ...
    for(const part of first) {
      ...
      if(!identifiedDigit) {
        ...
        if(intersections[532].length === 3) {
          ...
          // deduce_invalid_mappings(connections, intersection, [2, 3, 5]);
        }
        ...
        if(intersections[960].length === 3) {
          ...
          // deduce_invalid_mappings(connections, intersection, [0, 6, 9]);
        }
    ...
    const connectionsTranspose = transpose(connections);

    for(let i = 0; i < connections.length; i++) {
      const row = connections[i];
      const column = connectionsTranspose[i];
      const rowCount = row.reduce( (acc, value) => acc + (value ? 1 : 0), 0);
      const colCount = column.reduce( (acc, value) => acc + (value ? 1 : 0), 0);
      ...
      if(rowCount === 1) {
        ...
        //   if(j === index) continue;
        //   connections[index][j] = false;
        // }
      }

      if(colCount === 1) {
        const index = column.findIndex( (value) => value);
    ...
    for(const row of connections) {
      if(row.reduce( (acc, value) => acc + (value ? 1 : 0), 0) !== 1) {
        console.table(connections);
        console.log(connectionsTranspose[0]);
        console.log(connections[2]);
        throw new Error('We did not sufficiently deduce the wire mappings');
      }
    }
  ...
  if(output.length !== input.length) {
    throw new Error('Not correct amount of outputs');
  }
